...
bool _txGetCommandLine(wchar_t cmdLine[], size_t szCmdLine, unsigned pid /*= _getpid()*/)
{
	...
	$   *cmdLine = 0;
	if (ok && params.CommandLine.Buffer) {
		$ ok &= !!ReadProcessMemory(proc, params.CommandLine.Buffer, cmdLine,
...
HWND _txConsole_Attach()
{
	...
	{
		$ txReopenStdio();
	}                     // Переоткрываем потоки ввода-вывода, если subsystem != console
...
int _txOnRTCFailure(int type, const char* file, int line, const char* module, const char* format, ...)
{
	...
	$   int* errors = NULL;
	$   try { errors = (int*)_alloca(nErrors * sizeof(*errors)); }
	catch (...) { ; }
	...
	HANDLE thread /* = GetCurrentThread()*/)
{
	...
	$   for (frames = -framesToSkip; frames < (int)szCapture; frames++)
	{
		...
		if (!_TX_CALL(MinGW::StackWalk64, (cpu, process, thread, &frame, &ctx, NULL,
			MinGW::SymFunctionTableAccess64, MinGW::SymGetModuleBase64, NULL))) {
			$ break;
		}
#elif defined (_MSC_VER)

		$       if (!_TX_CALL(Win32::StackWalk64, (cpu, process, thread, &frame, &ctx, NULL,
			Win32::SymFunctionTableAccess64, Win32::SymGetModuleBase64, NULL))) {
			$ break;
		}
#else
...
int txOutputDebugPrintf(const char format[], ...)
{
	...
	struct __ {
		static int trimSpaces(char str[])
		{
...
void tx_fpreset()
{
	...
	{
		$(void) _controlfp_s(&old87, old87  & ~new87, 0x0008001F);
	}  // _MCW_EM
